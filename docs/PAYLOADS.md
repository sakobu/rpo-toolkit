...
| `velocity_ric_km_s` | no       | `null` (position-only) | RIC velocity target (km/s). See semantics below |
...
- **`[0, 0, 0]`:** Explicit station-keeping — arrive and hold with zero relative velocity.
- **Any other value:** Velocity-constrained target. Enrichment is advisory-only for that waypoint.
