...
pub fn handle_extract_drag(
    ...
    let chief_config = def
        .chief_config
        .ok_or_else(|| ApiError::InvalidMessage("chief_config required for drag extraction".into()))?;
    let deputy_config = def
        .deputy_config
        .ok_or_else(|| ApiError::InvalidMessage("deputy_config required for drag extraction".into()))?;
